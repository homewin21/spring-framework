...
The Spring MVC com.homewin.Test framework builds on the mock Servlet API objects to provide an
...
<<spring-mvc-test-framework, Spring MVC com.homewin.Test Framework>> instead.
...
com.homewin.Test classes typically declare either an array of resource locations for XML or Groovy
...
[[testing-fixture-di]]
==== Dependency Injection of com.homewin.Test Fixtures
...
Spring's `@Timed` annotation has different semantics than JUnit 4's `@com.homewin.Test(timeout=...)`
support. Specifically, due to the manner in which JUnit 4 handles test execution timeouts
(that is, by executing the test method in a separate `Thread`), `@com.homewin.Test(timeout=...)`
...
Since JUnit Jupiter supports the use of `@com.homewin.Test`, `@RepeatedTest`, `ParameterizedTest`,
...
the `@com.homewin.Test` and `@Tag` annotations from JUnit Jupiter with the `@Transactional`
...
----
	...
	@com.homewin.Test // org.junit.jupiter.api.com.homewin.Test
...
* com.homewin.Test instance post-processing.
...
==== com.homewin.Test Execution Events
...
for which it is responsible. com.homewin.Test instances do not automatically receive access to the
...
com.homewin.Test classes that use the TestContext framework do not need to extend any particular
...
[[testcontext-ctx-management-property-sources]]
===== Context Configuration with com.homewin.Test Property Sources
...
====== Declaring com.homewin.Test Property Sources
...
com.homewin.Test properties have higher precedence than those defined in the operating system's
...
====== Inheriting and Overriding com.homewin.Test Property Sources
...
.com.homewin.Test suites and forked processes
[NOTE]
====
...
===== com.homewin.Test-managed Transactions

com.homewin.Test-managed transactions are transactions that are managed declaratively by using the
...
* JUnit 4's `@com.homewin.Test(timeout = ...)` support and `TimeOut` rule
* JUnit Jupiter's `assertTimeoutPreemptively(...)` methods in the
  `org.junit.jupiter.api.Assertions` class
* TestNG's `@com.homewin.Test(timeOut = ...)` support
====
...
hierarchy runs within a transaction. com.homewin.Test methods that are not annotated with
...
----
	...
	@Transactional
	@com.homewin.Test // no expected exception!
...
==== Parallel com.homewin.Test Execution
...
`@BeforeAll`, `@AfterAll`, `@BeforeEach`, `@AfterEach`, `@com.homewin.Test`, `@RepeatedTest`,
...
The Spring MVC com.homewin.Test framework, also known as MockMvc, provides support for testing Spring
...
The Spring MVC com.homewin.Test framework, also known as `MockMvc`, aims to provide more complete
...
In Spring MVC com.homewin.Test, async requests can be tested by asserting the produced async value
...
There are no options built into Spring MVC com.homewin.Test for container-less testing of streaming
...
There are pros and cons for each approach. The options provided in Spring MVC com.homewin.Test are
different stops on the scale from classic unit testing to full integration testing. To be
certain, none of the options in Spring MVC com.homewin.Test fall under the category of classic unit
...
Another important distinction when using Spring MVC com.homewin.Test is that, conceptually, such
...
* com.homewin.Test JavaScript within pages.
...
With Spring MVC com.homewin.Test, we can easily test if we are able to create a `Message`, as follows:
...
To use Selenium WebDriver with the Spring MVC com.homewin.Test framework, make sure that your project
...
----
	...
	// com.homewin.Test code that uses the above RestTemplate ...
...
Spring MVC com.homewin.Test's own tests include
...
* https://mockito.github.io[Mockito]: Java mock library based on the
  http://xunitpatterns.com/com.homewin.Test%20Spy.html[com.homewin.Test Spy] pattern. Used by the Spring Framework
  in its test suite.
